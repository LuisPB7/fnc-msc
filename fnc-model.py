...
with open("cider_fnc.pkl", "rb") as ciderFile:
    cider_fnc_train = pickle.load(ciderFile)
    cider_fnc_test = pickle.load(ciderFile)
    cider_two_train = pickle.load(ciderFile)
    cider_two_test = pickle.load(ciderFile)
...
final_merge = merge([concat, mul, dif, input_overlap, input_refuting, input_polarity, input_hand, input_sim, input_bleu, input_rouge, input_cider], mode='concat')
...
final_model.fit([X1, X2, overlapFeatures_fnc, refutingFeatures_fnc, polarityFeatures_fnc, handFeatures_fnc,  \
                ...
                callbacks=[weightedAccuracy] + snapshot.get_callbacks(model_prefix=model_prefix), batch_size=32, nb_epoch=100, class_weight={0:1, 1:3, 2:3, 3:3})
